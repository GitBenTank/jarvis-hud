...
| `execution.dry_run` | Whether execute response is treated as preview-only vs persisted / outbound “real run” (`dryRun` in JSON). | `src/app/api/execute/[approvalId]/route.ts` — `dryRun: !isNonDryRunExecuteKind(executionKind)`; `src/lib/execution-surface.ts` — `NON_DRY_RUN_EXECUTE_KINDS`, `buildExecutionCapabilities()`; `GET /api/config` → `runtimePosture.executionCapabilities`, `trustPosture.executionSurfaceLabel` | Honest labeling **per kind** | — | — | — | **Yes** | **Surfaced** — no global “HUD is dry-run” claim |
...
| `73af4b2f-9fff-4c14-b2b9-daa3defa52e7` | What was produced; where proof lives (happy `system.note`) | **QA closed (2026-04-11):** Proposal → approval → ALLOW → execute → receipt/reconciliation legible; artifact path shown in lifecycle/timeline. **Still open for edge cases:** non-file kinds with **no** paths may need Ticket D **“Output: in-event note”** copy. | `TracePanel.tsx` receipt/artifact sections | For kinds without paths, add an explicit **“Output: in-event note”** (or similar) and surface `summary`/payload snippet so “produced” is legible. | Med (edge case only) |
...
1. ~~One successful **`system.note`** (happy path).~~ **Done (2026-04-11):** `73af4b2f-9fff-4c14-b2b9-daa3defa52e7` — full chain legible (see gap table row).
2. One **execute-time policy deny** with a **known logged reason** — validates mitigated policy lifecycle under the deny path (not human **reject** before execute).
3. One **older or noisy** trace (multi-event or missing actor fields), if available.

**How to produce item 2 (pick by `authEnabled` on `GET /api/config`):**

- **`authEnabled === false`** (typical local HUD + OpenClaw / Alfred): use **`code.apply`**. **Approve** the proposal, then click **Execute** while **`code.apply` preflight** will block (e.g. **dirty git worktree** or any other real preflight rule your env hits). That path logs execute-time policy deny and is the one that proves policy/trace UI in this environment.
- **`authEnabled === true`:** alternate trigger — **approve**, then **Execute** without valid **step-up** (policy denies before adapters).

**Substrate vs operator QA:** `pnpm test` / Vitest (`trace-view`, `policy`, trust posture, etc.) covers API and policy **substrate**; item 2 still needs a **real deny trace id** checked in the browser for operator-truth.
...
| Per-adapter dry run | `src/app/api/execute/[approvalId]/route.ts`, `src/lib/execution-surface.ts` | Execute response `dryRun` via `isNonDryRunExecuteKind`; strip summarizes surface from config |
...
- [OpenClaw v1 contract](./openclaw-v1-contract.md) — canonical agent/proposal posture read
- [OpenClaw ↔ Jarvis trust contract](./openclaw-jarvis-trust-contract.md) — bridge and HUD-specific trust notes
