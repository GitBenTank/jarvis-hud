...
**Operators:** the blessed local flow is **[Local stack startup](../setup/local-stack-startup.md)** — **Terminal 1** `pnpm dev` (**http://127.0.0.1:3000**), **Terminal 2** `OPENCLAW_ROOT=~/Documents/openclaw-runtime pnpm openclaw:dev`, then **`pnpm local:stack:doctor`**. Optional: **`pnpm dev:stack`** prints the same commands with env checks. **macOS:** [Raycast Script Commands](../setup/local-stack-startup.md#raycast-script-commands) mirror those steps (long runs open **Terminal.app**).
