...
| **S2 — Approve / execute persistence** | `resolveGovernedHumanPrincipal` → `POST /api/approvals/:id` + execute persist `approvalActor*` / `executionActor*` + `approvalPrincipalIss`/`Sub` / `executionPrincipalIss`/`Sub` when bound; receipts use event fields; **403** if binding required and unbound. **Golden-loop:** stub-bind between init and step-up when `JARVIS_IDENTITY_BINDING_REQUIRED` (`GOLDEN_LOOP_OIDC_*` + allowlist). | `tests/unit/governed-human-principal.test.ts`; F1/F2 on stack when binding on. |
