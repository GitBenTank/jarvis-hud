...
| `GOLDEN_LOOP_OIDC_ISS` | When `pnpm golden-loop` with binding required | Issuer string for stub-bind; must appear in `JARVIS_OIDC_ISSUER_ALLOWLIST`. |
| `GOLDEN_LOOP_OIDC_SUB` | Same | OIDC `sub` for stub-bind. |
