...
- **Say:** Name the three forces on slide 2; **spoken:** if the opener already listed email/code/API examples, **don’t re-read Capability aloud**—let the bullet read on-screen, then verbalize Ungated execution + Governance pressure. On **slide 3** tie in **`system.note`** + risk-class anchor with the typewriter; then live / OpenClaw / ingress. Full weave: [investor-demo-narrative-script.md](./investor-demo-narrative-script.md).
