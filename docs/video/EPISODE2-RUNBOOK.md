...
- **Browser** → Jarvis UI (http://127.0.0.1:3001)
...
**Wait for:** `Ready` or `Local: http://127.0.0.1:3001`

**Success criteria:** Jarvis UI loads at http://127.0.0.1:3001. `pnpm jarvis:doctor` shows ingress enabled, secret present, allowlist includes openclaw.
...
| 403 Forbidden | Wrong `JARVIS_BASE_URL` or ingress disabled | Set `JARVIS_BASE_URL=http://127.0.0.1:3001` (OpenClaw). For jarvis-hud smoke use `JARVIS_HUD_BASE_URL`. Run `pnpm jarvis:doctor` and restart Jarvis with env from `demo-env.sh`. |
...
| `JARVIS_HUD_BASE_URL` | jarvis-hud scripts (ingress:smoke) | `http://127.0.0.1:3001` |
| `JARVIS_BASE_URL` | OpenClaw (jarvis:smoke) | `http://127.0.0.1:3001` |
