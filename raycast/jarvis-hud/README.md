...
1. Install [Raycast](https://www.raycast.com/) if you have not already (download / updates; **not** where you register Script Directories).
2. In the **Raycast Mac app** → **Settings** → **Extensions** → **Script Commands** → **Script Directories** → **Add Directories** (the website has no Script Directories UI).
3. Add the **inner** folder that contains `_env.sh` and `kill-local-stack.sh`: `<your-clone>/raycast/jarvis-hud` (or symlink that folder into e.g. `~/raycast-scripts/jarvis-hud`). If the folder picker is awkward, use **Go → Go to Folder…** (**⌘⇧G**) and paste the path.
